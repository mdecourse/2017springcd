...
# inputs
ax, ay, bx, by, cab, ac = symbols('ax ay bx by cab ac')
# intermediate variables
d, bad, degree= symbols('d bad degree')
ad, bd = symbols('ad bd')
# outputs
cx, cy = symbols('cx cy')
# 從 a, b 點座標求 d, ad 與 bd
...
pos = 1
if pos == 0:
    bad = solve(-bd**2+ad**2+d**2-2*ad*d*cos(bad), bad)[0]
else:
    bad = solve(-bd**2+ad**2+d**2-2*ad*d*cos(bad), bad)[1]
#print(bad)
...
def bad(ax, ay, bx, by, pos):
    if pos == 0:
        return -acos(-(ax - bx)/sqrt(ax**2 - 2*ax*bx + ay**2 - 2*ay*by + bx**2 + by**2)) + 2*pi
    else:
        return acos((-ax + bx)/sqrt(ax**2 - 2*ax*bx + ay**2 - 2*ay*by + bx**2 + by**2))

radian = 180/pi
degree = pi/180

def cpoint(ax, ay, bx, by, cab, ac):
    cx = ac*cos(cab + acos((-ax + bx)/sqrt(ax**2 - 2*ax*bx + ay**2 - 2*ay*by + bx**2 + by**2)))
    cy = ac*sin(cab + acos((-ax + bx)/sqrt(ax**2 - 2*ax*bx + ay**2 - 2*ay*by + bx**2 + by**2)))
    return cx, cy

#print("bad 角度為", bad(0, 0, 100, 56, 1)*radian, "度")

cx, cy = cpoint(0, 0, 100, 56, 30*degree, 30)
print("cx=", cx, "cy=", cy)
</pre>